...
public class TzrpcAutoRegister implements CommandLineRunner {
    ...
    private List<ServiceProxy<?, ?>> loadServiceProxy() {
        ...
        Map<Class<?>, ServiceProxy<?, ?>> services = new HashMap<>();
        for(Object provider : providers.values()) {
            ...
            for(Class<?> interf : interfs) {
                ...
                ServiceProxy<?, Object> serviceProxy = new ServiceProxy<>(interf, provider);
                ...
                services.put(interf, serviceProxy);
            }
        }
